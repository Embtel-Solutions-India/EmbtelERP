...
import {
  fetchDocumentationTasks, addDocumentationTask, updateDocumentationTask, deleteDocumentationTask,
} from '../../../../redux/slices/documentationTaskSlice'
import {
...
export default function DocumentationTasks() {
  const dispatch = useDispatch()
  const { list: tasks, loading, error } = useSelector((s) => s.documentationTasks)
  ...
  useEffect(() => {
    dispatch(fetchDocumentationTasks())
  ...
  useEffect(() => {
    ...
    let active = true
    documentationTaskService.getAssignableUsers()
  ...
  const handleSubmit = async (values) => {
    ...
    if (editingTask) {
      await dispatch(updateDocumentationTask({ id: editingTask.id, ...payload }))
    } else {
      await dispatch(addDocumentationTask(payload))
    }
    setFormOpen(false)
    setEditingTask(null)
  }

  const handleComplete = (task) => dispatch(updateDocumentationTask({ id: task.id, status: 'COMPLETED' }))
  const handleDelete = (id) => { if (window.confirm('Delete this task?')) dispatch(deleteDocumentationTask(id)) }

  return (
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <PageHeader
        title="Tasks"
        subtitle={`${visibleTasks.length} documentation tasks`}
      ...
      <SchemaForm
        ...
        subtitle={editingTask ? 'Update task and outcome' : 'Create a documentation follow-up task'}
        ...
        <div className="card p-10 text-center">
          ...
          <p className="mt-1 text-xs text-neutral-400">
            {canAssign && activeTab === 'assigned'
              ...
              : 'Create a documentation task to keep your cases moving.'}
          </p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-neutral-100 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-700/50">
                  {['Task ID', 'Title', 'Type', 'Due', 'Priority', 'Status', 'Result', 'Next Follow-up', 'Actions'].map((h) => (
