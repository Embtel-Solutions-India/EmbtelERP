...
import {
  FaBullhorn, FaEnvelope, FaShareAlt, FaFileAlt,
  FaVideo, FaCalendarAlt,
} from 'react-icons/fa'
import { cancelBroadcast } from '../redux/marketingEmailSlice'
import { formatDate, formatTime, getInitials } from '../../../utils'
import SectionCard from '../../../components/common/SectionCard'

const TYPE_MAP = {
  'PPC Launch':      { Icon: FaBullhorn, color: 'text-indigo-500'  },
  'Email Broadcast': { Icon: FaEnvelope, color: 'text-cyan-500'    },
  'Social Post':     { Icon: FaShareAlt, color: 'text-amber-500'   },
  'Content Release': { Icon: FaFileAlt,    color: 'text-emerald-500' },
  'Webinar Event':   { Icon: FaVideo,      color: 'text-purple-500'  },
}
...
export default function MarketingLaunchCards() {
  const dispatch = useDispatch()
  const { list: campaigns } = useSelector((s) => s.marketingEmails)
  const upcoming = campaigns.filter((m) => m.status === 'Scheduled').slice(0, 4)

  return (
    <SectionCard
      title="Upcoming Launches & Broadcasts"
      subtitle="Scheduled marketing activities"
      delay={0.2}
      actions={<span className="badge badge-info">{upcoming.length} scheduled</span>}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  {meeting.link && (
                    ...
                    >
                      <VideoCall style={{ fontSize: 13 }} /> View Tool
                    </a>
                  )}
                  ...
                  <button
                    onClick={() => dispatch(cancelBroadcast(meeting.id))}
