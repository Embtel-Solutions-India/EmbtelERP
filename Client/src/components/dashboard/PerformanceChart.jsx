      ...
      {payload.map((p) => (
        <div key={p.name} className="flex items-center gap-2 text-xs">
          ...
          <span className="font-bold text-slate-700 dark:text-slate-200">
            {p.name === 'revenue' || p.name === 'target' ? formatCurrency(p.value) : p.value}
          </span>
        </div>
      ))}
    </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          {chartTab === 'Revenue' ? (
            <AreaChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="target"  stroke="#10b981" strokeWidth={2}   fill="url(#tgtGrad)" strokeDasharray="5 5" dot={false} activeDot={{ r: 4, fill: '#10b981' }} />
            </AreaChart>
          ) : chartTab === 'Leads' ? (
