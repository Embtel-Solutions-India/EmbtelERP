...
export async function createEmployee(input, actorId) {
    ...
    if (!employee.employeeCode) {
        const businessCode = business && (business.code || business.name)
...
export async function updateEmployee(id, input, actorId) {
...
export async function deactivateEmployee(id, actorId) {
    const existing = await prisma.employee.findUnique({ where: { id } });
    if (!existing) {
        throw new ApiError(404, "Employee not found");
    }
