...
export async function getAvailablePerspectives(userId) {
    ...
    const tree = await buildHierarchyTree(viewer, viewerBusinessIds, allVisibleIds);
    return {
        currentPerspective,
 ...
 */
export async function switchPerspective(userId, targetType, targetId) {
    const viewer = await prisma.employee.findUnique({
        where: { id: userId },
 ...
 */
export async function clearPerspective(userId) {
    await prisma.perspectiveSession.deleteMany({ where: { userId } });
}
/**
 * Validate that a user can access a given perspective target.
 */
export async function validatePerspectiveAccess(userId, targetType, targetId) {
    const viewer = await prisma.employee.findUnique({
        where: { id: userId },
        select: {
            id: true,
    ...
    switch (targetType) {
        case "ORGANIZATION": {
            ...
            if (viewer.role.level < 5) {
                throw new ApiError(403, "Access denied: cannot view organization scope");
            }
            return true;
        }
        case "BUSINESS": {
        ...
        case "VERTICAL": {
            ...
            if (!vertical)
                throw new ApiError(404, "Vertical not found");
        ...
        case "HEAD": {
            ...
            if (!headEmployee)
                throw new ApiError(404, "Head employee not found");
        ...
        case "BUSINESS_OWNER": {
            ...
            if (viewer.role.level < 4) {
                throw new ApiError(403, "Access denied: Business Owner perspective requires level 4+");
            }
            const business = await prisma.business.findUnique({
                where: { id: targetId },
                select: { organizationId: true },
 ...
 */
export async function getPerspectiveInfo(userId) {
    const session = await getActivePerspectiveForUser(userId);
    if (!session)
        return null;
    const breadcrumb = await buildBreadcrumb(session.perspectiveType, session.perspectiveTargetId);
    return {
        id: session.id,
...
function employeeNodeType(level) {
    if (level === 3)
        return "HEAD";
    if (level === 2)
        return "MANAGER";
...
async function buildHierarchyTree(viewer, businessIds, visibleEmployeeIds) {
    const tree = [];
    ...
    for (const business of businesses) {
        ...
        const businessNodeType = viewerLevel >= 4 ? "BUSINESS_OWNER" : "BUSINESS";
        const businessNode = {
            id: business.id,
...
async function buildBreadcrumb(type, targetId) {
    const breadcrumb = [];
    switch (type) {
        case "ORGANIZATION": {
...
async function validateScopeBoundaries(viewer, targetType, targetId) {
    const viewerLevel = viewer.role.level;
    ...
    if (targetType === "VERTICAL") {
        ...
        if (!target)
            throw new ApiError(404, "Target vertical not found");
        // Head of Immigration
