 ...
 * level.
 */
export async function resolveAggregationScope(scope, perspective) {
    if (!perspective) {
        return {
    ...
    switch (type) {
        ...
        case "VERTICAL": {
            ...
            if (!vertical)
                return { employeeIds: [], teamIds: [], businessIds: [] };
            const [members, teams] = await Promise.all([
                prisma.employee.findMany({
        ...
        case "TEAM": {
            ...
            if (!team)
                return { employeeIds: [], teamIds: [targetId], businessIds: [] };
            const members = await prisma.employee.findMany({
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
async function getPerspectiveLabel(type, targetId) {
    switch (type) {
        case "BUSINESS":
        case "BUSINESS_OWNER": {
...
function determineAggregationLevel(type, viewerRoleLevel) {
    if (viewerRoleLevel >= 5)
        return "ALL";
    switch (type) {
        case "ORGANIZATION":
            return "ALL";
...
async function getTaskStats(employeeIds, businessIds) {
...
async function getMarketingKpiStats(businessIds, teamIds, employeeIds) {
...
function hasDesignation(designation, ...needles) {
    return needles.some((needle) => designation.includes(needle));
}
function completionRate(completed, total) {
    return total > 0 ? Math.round((completed / total) * 100) : 0;
}
function makeWidget(key, title, value, label, tone = "neutral") {
    return { key, title, value, label, tone };
}
function makeAction(key, label, path) {
    return { key, label, path };
}
// ─── Main Dashboard Service ──────────────────────────────────────────────────
export async function getDashboardOverview(scope, perspective) {
    const resolved = await resolveAggregationScope(scope, perspective);
    const { employeeIds, teamIds, businessIds } = resolved;
    const [employeeCount, activityCount, auditCount, taskStats, marketingStats] = await Promise.all([
    ...
    if (perspective?.type === "TEAM" ||
        (perspective?.type === "MANAGER" &&
            (await prisma.employee
                ...
                .then((e) => !!e?.teamId)))) {
        const teamId = perspective?.type === "TEAM"
    ...
    if (perspective?.type === "EMPLOYEE" ||
        ...
        if (employee) {
            ...
            employeeKpis = {
                ...
                performanceScore: empStats.total > 0
                    ? Math.round((empStats.completed / empStats.total) * 100)
                    : 0,
                productivity: empStats.total > 0
    ...
    return {
        ...
        perspective: perspective
            ? {
                ...
                aggregationLevel: determineAggregationLevel(perspective.type, viewer?.role.level ?? 0),
            }
            : null,
        teamKpis,
        employeeKpis,
...
export async function getDashboardPerformance(scope, perspective) {
    const { employeeIds, businessIds } = await resolveAggregationScope(scope, perspective);
    const whereOr = [];
    if (businessIds.length > 0)
        whereOr.push({ businessId: { in: businessIds } });
    if (employeeIds.length > 0)
        whereOr.push({ employeeId: { in: employeeIds } });
    const kpis = whereOr.length > 0
    ...
    const tasks = taskWhereOr.length > 0
    ...
    const entry = (key) => monthlyMap.get(key) ?? {
        revenue: 0,
        target: 0,
        leads: 0,
        conversions: 0,
    ...
    for (const kpi of kpis) {
        ...
        const e = entry(key);
        if (kpi.metricType === "LEADS_GENERATED")
            e.leads += Number(kpi.value);
        if (kpi.metricType === "CAMPAIGN_SUCCESS")
            e.conversions += Number(kpi.value);
        if (kpi.target)
            e.target += Number(kpi.target);
        if (kpi.metricType === "BUDGET_UTILIZATION")
            e.revenue += Number(kpi.value);
    ...
    return [...monthlyMap.entries()]
        ...
        employeeProductivity: d.tasksCreated > 0
            ? Math.round((d.tasksCompleted / d.tasksCreated) * 100)
            : 0,
    }));
}
export async function getDashboardInsights(scope, perspective) {
    const { employeeIds, teamIds, businessIds } = await resolveAggregationScope(scope, perspective);
    const insights = [];
    ...
    if (teamIds.length > 0)
        kpiFilter.push({ teamId: { in: teamIds } });
    if (employeeIds.length > 0)
        kpiFilter.push({ employeeId: { in: employeeIds } });
    const kpis = kpiFilter.length > 0
        ? await prisma.marketingKPI.findMany({
            where: { OR: kpiFilter },
            select: {
                value: true,
    ...
    // 5. Revenue trend
    const revenueKpis = kpis.filter((k) => k.metricType === "LEADS_GENERATED" ||
        k.metricType === "CAMPAIGN_SUCCESS");
    if (revenueKpis.length >= 2) {
        const avg = revenueKpis.reduce((s, k) => s + Number(k.value), 0) /
            revenueKpis.length;
        insights.push({
            type: "neutral",
            category: "Revenue",
...
export async function getDashboardTeam(scope, perspective) {
    const { teamIds, businessIds } = await resolveAggregationScope(scope, perspective);
    if (teamIds.length === 0 && businessIds.length === 0)
        return [];
    const teamFilter = [];
    if (teamIds.length > 0)
        teamFilter.push({ id: { in: teamIds } });
    if (businessIds.length > 0)
        teamFilter.push({ businessId: { in: businessIds } });
...
export async function getRoleWorkspace(scope, viewer, perspective) {
    const { employeeIds, teamIds, businessIds } = await resolveAggregationScope(scope, perspective);
    const employee = await prisma.employee.findUnique({
    ...
    const budgetUsage = budget.allocated > 0 ? Math.round((budget.spent / budget.allocated) * 100) : 0;
    ...
    if (hasDesignation(designation, "marketing")) {
        ...
        return {
            roleKey,
            title: roleKey === "marketing_manager"
