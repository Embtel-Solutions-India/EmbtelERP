...
import { getAvailablePerspectives, switchPerspective, getActivePerspectiveForUser, } from "../services/perspective.service.js";
...
perspectivesRouter.post("/switch", validateBody(switchSchema), asyncHandler(async (req, res) => {
    const perspective = await switchPerspective(req.user.employeeId, req.body.targetUserId);
    res.json({ data: perspective });
}));
perspectivesRouter.post("/reset", asyncHandler(async (req, res) => {
    // reset perspective to self
    const perspective = await switchPerspective(req.user.employeeId, req.user.employeeId);
    res.json({ data: perspective });
}));
